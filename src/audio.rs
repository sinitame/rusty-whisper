...
impl Featurizer {
    ...
    pub fn featurize(&self, audio_data: &[f32]) -> Array2<f32> {
        let padded_audio = Self::pad_audio(audio_data);
        let stft = Self::par_generate_stft(&padded_audio, 400, 160);
        let magnitudes: Array2<f32> =
            Array2::from_shape_fn((stft[0].len(), stft.len()), |(i, j)| {
                let element = stft[j][i].abs();
                element * element
            });
        let mel_spec = self.filters.dot(&magnitudes);
    ...
    fn par_generate_stft(audio: &[f32], n_fft: usize, hop_length: usize) -> Vec<Vec<Complex<f32>>> {
        let window = Self::generate_hann_window(n_fft);
